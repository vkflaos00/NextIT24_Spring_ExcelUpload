...
public class MemberController {
	...
	public String memberRegister(@Validated(value = MemberRegister.class) @ModelAttribute("member") MemberVO member,
		...
		try {
			...
			bde.printStackTrace();
			resultMessageVO.failSetting(false, "회원등록실패", "이미 사용중인 아이디 입니다. 다른 아이드를 사용해주세요");
	...
	public String memberEdit(@RequestParam String memId, Model model) {
	...
	public String memberModify(@Validated(value = MemberModify.class) @ModelAttribute("member") MemberVO member,
			BindingResult error, Model model, ResultMessageVO resultMessageVO,
			@RequestParam(required = false) MultipartFile[] profilePhoto) {
		...
		try {
			...
			if (fileUploadflag) {
				...
				resultMessageVO.failSetting(false, "파일 업로드 실패",
						"회원정보는 수정되었으나 파일이 업로드 되지 못하였습니다. 전산실에 문의 부탁드립니다. 042-719-8850");
			}

		} catch (BizNotEffectedException bne) {
			resultMessageVO.failSetting(false, "회원정보 수정 실패", "회원정보 수정 실패 하였습니다. 전산실에 문의 부탁드립니다. 042-719-8850");
			bne.printStackTrace();
		} catch (BizPasswordNotMatchedException bpn) {
			resultMessageVO.failSetting(false, "회원정보 수정 실패", "입력하신 패스워드가 일치하지 않습니다. 다시 입력 해주세요");
			bpn.printStackTrace();
	...
	public String memberDelete(@ModelAttribute MemberVO member, HttpServletRequest request, Model model,
			ResultMessageVO resultMessageVO) {
		System.out.println("MemberController memberDelete member.toString() :" + member.toString());
		...
		try {
			...
			resultMessageVO.failSetting(false, "회원탈퇴 실패", "회원탈퇴 실패하였습니다. 전산실에 문의 부탁드립니다. 042-719-8850");
			bnf.printStackTrace();
		} catch (BizPasswordNotMatchedException bpn) {
	...
	public String memberMultiDelete(@RequestParam String memMultiId, Model model, ResultMessageVO resultMessageVO) {
		...
		try {
			...
			bne.printStackTrace();
			// model.addAttribute("bne", bne);
			resultMessageVO.failSetting(false, "회원삭제 실패", "회원삭제 실패하였습니다. 전산실에 문의 부탁드립니다. 042-719-8850");
		} catch (Exception de) {
			de.printStackTrace();
			// model.addAttribute("de", de);
			resultMessageVO.failSetting(false, "회원삭제 실패", "회원삭제 실패하였습니다. 전산실에 문의 부탁드립니다. 042-719-8850");
	...
	public String memberRoleUpdate(@RequestParam String memId,
		...
		try {
			...
			return "redirect:/member/memberList";
			/*
			 * }catch(BizNotEffectedException bne){ //model.addAttribute("bne", bne);
			 * resultMessageVO.failSetting(false , "회원 권한 수정 실패" ,
			 * "회원 권한 수정 실패 하였습니다. 전산실에 문의 부탁드립니다. 042-719-8850"); bne.printStackTrace();
			 */
		} catch (Exception de) {
			// model.addAttribute("de", de);
			resultMessageVO.failSetting(false, "회원 권한 수정 실패", "회원 권한 수정 실패 하였습니다. 전산실에 문의 부탁드립니다. 042-719-8850");
			de.printStackTrace();
		}
	...
	public String memberExcelUpload(@RequestParam(required = true) MultipartFile memberExcelUpload,
			ResultMessageVO resultMessageVO, Model model) {

		String path1 = "/home/pc21/upload/excel";
		...
		try {
			...
			e.printStackTrace();
			resultMessageVO.failSetting(false, "회원 엑셀 등록 실패", "회원 엑셀 등록 실패했습니다. 전산실에 문의 부탁드립니다. 042-719-8850");
		}
		model.addAttribute("resultMessageVO", resultMessageVO);
	...
	// 각 시트에서 첫번째 행과 마지막 행을 취득하는 용도의 메소드입니다 ^.^
	...
	private void getRow(Sheet sheet, int startRow, int endRow) throws BizNotEffectedException {
		...
		for(int i =startRow; i<=endRow; i++) {
			...
			if(row != null) {
				if(i== startRow) {
					cellCnt = row.getLastCellNum(); //해당 셀 개수 취득
				}else {
	...
	/*
	 * @RequestMapping("/member/memberExcelDownload") public void
	 * memberExcelDownload(HttpServletResponse response) throws IOException {
	 * 
	 * XSSFWorkbook wb = null; Sheet sheet = null; Row row = null; Cell cell = null;
	 * 
	 * wb = new XSSFWorkbook(); sheet = wb.createSheet("NextIT 회원 목록");
	 * 
	 * //row 생성 row = sheet.createRow(0); // 0번째 행 cell = row.createCell(0);
	 * cell.setCellValue("순번"); cell = row.createCell(1); cell.setCellValue("아이디");
	 * cell = row.createCell(2); cell.setCellValue("이름");
	 * 
	 * //row 생성 row = sheet.createRow(1); // 1번째 행 cell = row.createCell(0);
	 * cell.setCellValue("1"); cell = row.createCell(1);
	 * cell.setCellValue("nextit11"); cell = row.createCell(2);
	 * cell.setCellValue("넥스트아이티11");
	 * 
	 * //row 생성 row = sheet.createRow(2); // 2번째 행 cell = row.createCell(0);
	 * cell.setCellValue("2"); cell = row.createCell(1);
	 * cell.setCellValue("nextit12"); cell = row.createCell(2);
	 * cell.setCellValue("넥스트아이티12");
	 * 
	 * response.setContentType("ms-vnd/excel"); response.setHeader(
	...
	public void memberExcelDownload(HttpServletResponse response, @ModelAttribute MemberSearchVO searchVO) throws IOException {
		
		logger.info("memberExcelDownload searchVO.toString() :"+ searchVO.toString());
		...
		try {
			...
			if(memberList != null) {
				...
				wb = new XSSFWorkbook(); 
				sheet = wb.createSheet("NextIT 회원 목록");
